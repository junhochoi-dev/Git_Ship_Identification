...
export default class ListCommonShip extends Component{
	...
	updateCommonShipList(){
		getToken().then((token) => {
			requestCommonShipList(token, this.state.index).then((response) => {
			if(response.status == 200){
				this.setState({
					pages: response.data.data.count,
		...
		return(
			<base.Container>
				...
				<base.Content padder contentContainerStyle={{ flex: 1 }}>
					...
					/>
					<base.Form style={{flexDirection: 'row', alignItems: 'center', justifyContent: 'center', height: 50}}>
